 ...
 try {
    ...
    products?.length
         ? res.json({
           ...
           result: products.map(({ _id, name, alternateName, parentCategoryID, qrCodeNumber, price, salesPrice, thumbnail }) =>
             ({ name, _id, alternateName, parentCategoryID, qrCodeNumber, price, salesPrice, thumbnail }))
          })
        : res.json({
            status: "success",
